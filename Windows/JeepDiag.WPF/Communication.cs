...
namespace JeepDiag.WPF
{
    public class Communication : ICommunication
    {
        ...
        public const string EmulatorPortName = "Emulator";  

        public static IDictionary<string, string> GetSerialPortNames()
        {
            var portObjects = new ManagementObjectSearcher("Select * from Win32_SerialPort").Get()
                .Cast<ManagementObject>().ToList();
            var ports = new Dictionary<string, string>();
            foreach (var portName in SerialPort.GetPortNames())
            {
                var po = portObjects.Where(po => portName.Equals(po["DeviceID"].ToString(), StringComparison.OrdinalIgnoreCase))
                    .Select(po => po["Name"].ToString())
                    .SingleOrDefault();
                ports.Add(portName, po ?? string.Empty);
            }
            ports.Add(EmulatorPortName, "PCM Emulator");
            return ports;
        }
        ...
        public bool HasToShowSerialPortDialog()
        {
            if (string.IsNullOrEmpty(SerialPortName))
                return true;
            if (!GetSerialPortNames().ContainsKey(SerialPortName))
                return true;
            return false;
        }
        ...
        public async Task<byte[]> SendRequest(byte[] data)
        {
            ...
            int read = _serialPort.Read(_readBuffer, 0, _readBuffer.Length);
            if (read == 0)
        ...
        public async Task<List<string>> RequestStoredDTC()
        {
            var data = await SendRequest(new byte[] { 0x10 });
            if (data.Length < 3)
                throw new CommunicationException("SCI-bus error");

            if (Checksum(ref data, data.Length - 1) == data[data.Length - 1])
                throw new CommunicationException("Checksum error");
            ...
            for (int i = 1; i < (data.Length - 2); i++)
                if (DTCs.TryGetValue(data[i], out var dtc))
                    dtcs.Add(dtc);

            return dtcs;
        }

        public async Task<List<string>> RequestPendingDTC()
        {
            var data = await SendRequest(new byte[] { 0x11 });
            if (data.Length < 3)
                throw new CommunicationException("SCI-bus error");

            var dtcs = new List<string>(2);
        ...
        public async Task<string> ResetDTCs()
        {
            var data = await SendRequest(new byte[] { 0x17 });
            if (data.Length < 1)
                throw new CommunicationException("SCI-bus error");
        ...
        private static int Checksum(ref byte[] data, int length)
        {
            byte cs = 0;
        ...
        public static readonly IDictionary<byte, string> DTCs = new Dictionary<byte, string>()
        {
